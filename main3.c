...
/*   Updated: 2020/07/27 04:09:30 by pcariou          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	insert_sprite(map_list *elem, int *data_bi, int j)
{
	...
	while (x < elem->sprites[j].height && (int)elem->sprites[j].left < elem->x)
	{
		...
		while (y < elem->sprites[j].height && (int)top < elem->y)
		{
			...
			if (elem->sprite.data_img[(int)ratio_h + elem->sprite.width
				* (int)ratio_v] != elem->sprite.data_img[0])
			{
				if ((int)elem->sprites[j].left >= 0 && elem->dist
					[(int)elem->sprites[j].left] > elem->sprites[j].dist)
					data_bi[((int)elem->sprites[j].left + elem->x * (int)top)] =
					elem->sprite.data_img[(int)ratio_h + elem->sprite.width *
					(int)ratio_v];
			}
			top++;
			y++;
